...
\scriptsize
\Begin{
\tcc*[l]{Monitor both the distance and the angle of the overlay node.}
$idealDist = |0.5 \cdot abDist|$\;
$perpDist = |\sin (angA) \cdot acDist|$\;
\lIf{$angA > \frac{\pi}{2}$ {\bf or} $angA + angC < \frac{\pi}{2}$ {\bf or} $perpDist > abDist$} {$likelihood = 0$\;}
...
\begin{itemize}
	...
	\item Discover any node within the region that has not be discovered
	\item Update region information every time a new region node is discovered
	\item Overlay node may use similar route to sensor
\end{itemize}
\end{column}
...
\lIf{$dist <= minDiste$} {$likelihood = 0$\;}
...
\begin{column}{.5\textwidth}
\begin{figure}
\includegraphics[height=\textwidth,angle=-90]{ideal_distant_path}
...
\begin{column}{.5\textwidth}
\begin{figure}
\includegraphics[height=.5\textwidth,angle=-90]{path_similarity_two_destinations}
\caption{Definition of path similarity presented in W. Sun, ``A Method for Overlay Network Latency Estimation from Previous Observation'', ICN-2013. $geo_similarity(v_0, v_1, v_2, v_3) = \min[dist(v_0, v_2) + dist(v_1, v_3), dist(v_0, v_3) + dist(v_1, v_2)]$}
\end{figure}
\end{column}

\begin{column}{.5\textwidth}
\begin{figure}
\includegraphics[height=.5\textwidth,angle=-90]{path_similarity_one_destination}
\caption{When considering one server, $geo\_similarity(v_0, v_1, v_2, v_3) = dist(v_0, v_2) = geo(v_0, v_2)$  Thus, this heuristic considers overlay peers located further away to be more likely candidates.}
\end{figure}
\end{column}
...
\caption{The intuition of this heuristic is to contact nearby overlay nodes to help assess what paths are available as distant nodes provide many more options.  We found, however, that this approach does not work well in practice due to the path similarity inherent with nearby nodes.}
\end{figure}
\end{column}
